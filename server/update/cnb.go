...
func newCNBProvider(repositoryURL, branch, app string, client *http.Client) (*cnbProvider, error) {
	...
	return &cnbProvider{repositoryURL: repositoryURL, branch: branch, app: app, client: client}, nil
}

func (p *cnbProvider) Name() string { return "cnb" }

func (p *cnbProvider) Check(ctx context.Context, req updater.CheckRequest) (*updater.Release, error) {
	...
	return &updater.Release{
		...
		Artifact: updater.Artifact{Filename: artifact.Filename, Filetype: filetypeOf(artifact.Filename), Size: artifact.Size, Platform: req.Platform, Arch: req.Arch},
		Verification: &updater.Verification{DigestAlgo: "sha256", Digest: digest},
		Metadata: map[string]any{"cnb.release.tag": manifest.Tag},
	}, nil
}

func (p *cnbProvider) Download(ctx context.Context, release *updater.Release, dst io.Writer, onProgress func(written, total int64)) error {
	...
	for {
		...
		if n > 0 {
			...
			if wn != n { return io.ErrShortWrite }
			written += int64(n)
			if onProgress != nil { onProgress(written, total) }
		}
...
func (p *cnbProvider) fetchLatestManifest(ctx context.Context) (*cnbUpdateManifest, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, p.latestManifestURL(), nil)
	if err != nil { return nil, err }
	req.Header.Set("Accept", "application/json")
	req.Header.Set("Cache-Control", "no-cache")
	resp, err := p.client.Do(req)
	if err != nil { return nil, fmt.Errorf("cnb: fetch manifest: %w", err) }
	defer resp.Body.Close()
...
func (p *cnbProvider) validateManifest(manifest *cnbUpdateManifest) error {
	if manifest.SchemaVersion != cnbManifestSchemaVersion { return fmt.Errorf("cnb: unsupported manifest schema %d", manifest.SchemaVersion) }
	if manifest.App != p.app { return fmt.Errorf("cnb: manifest app %q does not match %q", manifest.App, p.app) }
	if !validSemver(manifest.Version) { return fmt.Errorf("cnb: invalid manifest version %q", manifest.Version) }
	expectedTag := p.app + "-v" + manifest.Version
	if manifest.Tag != expectedTag { return fmt.Errorf("cnb: manifest tag %q does not match %q", manifest.Tag, expectedTag) }
	return nil
}

func (p *cnbProvider) latestManifestURL() string {
	return p.repositoryURL + "/-/git/raw/" + url.PathEscape(p.branch) + "/" + url.PathEscape(p.app) + "/latest.json"
}

func (p *cnbProvider) releaseDownloadURL(tag, filename string) string {
...
func findCNBArtifact(artifacts []cnbUpdateArtifact, kind, platform, arch string) (cnbUpdateArtifact, bool) {
	for _, artifact := range artifacts {
		if artifact.Kind == kind && artifact.Platform == platform && artifact.Arch == arch { return artifact, true }
	}
...
func expectedRuntimeFilename(app, platform, arch string) string {
	...
	case "darwin": return base + ".zip"
	case "linux": return base
	default: return ""
	}
}
...
func decodeSHA256(value string) ([]byte, error) {
	...
	if err != nil { return nil, err }
	if len(digest) != 32 { return nil, fmt.Errorf("expected 32 bytes, got %d", len(digest)) }
	return digest, nil
}
...
func filetypeOf(filename string) string {
	if index := strings.LastIndex(filename, "."); index >= 0 { return strings.ToLower(filename[index+1:]) }
	return ""
}
